...
export function FilterBar({ selected, setSelected }: FilterBarProps) {
  ...
  const renderDropdown = (label: string, key: keyof typeof selected, items: ReadonlyArray<string>) => (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" className="capitalize bg-foreground border-accent-foreground rounded-none text-card">
          {label}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="bg-foreground border-accent-foreground text-card w-auto p-2 space-y-2">
        {items.map(item => (
          <div key={item} className="flex bg-foreground border-accent-foreground text-card items-center space-x-2">
            <Checkbox
            ...
            />
            <Label htmlFor={`${key}-${item}`} className="capitalize border-accent-foreground text-card">
              {item}
            </Label>
          </div>
        ))}
